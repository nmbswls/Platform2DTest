...
public class PlatformController : MonoBehaviour
{
    ...
    public PlatformMoveState NormalState { get; protected set; }
    /// the initial parameters
    [Header("Default Parameters")]
    public PlatformMoveParameters DefaultParameters;
    /// the current parameters
    public PlatformMoveParameters Parameters { get { return _overrideParameters ?? DefaultParameters; } }
    ...
    public DetachmentMethods DetachmentMethod = DetachmentMethods.Object;
    ...
    protected PlatformMoveParameters _overrideParameters;
    ...
    protected MovingPlatform _movingPlatform = null;
    ...
    protected RaycastHit2D[][] _belowHitsAllStorage;
    ...
    protected MovingPlatform _movingPlatformTest;
    ...
    protected virtual void Initialization()
    {
        ...
        NormalState = new PlatformMoveState();
        ...
        _belowHitsAllStorage = new RaycastHit2D[NumberOfVerticalRays][];
    ...
    protected virtual void InitLayerMask()
    {
        ...
        OneWayPlatformMask = 1<<LayerMask.NameToLayer("OneWayPlatforms");
        MovingPlatformMask = 1<<LayerMask.NameToLayer("MovingPlatforms");
        MovingOneWayPlatformMask = 1<<LayerMask.NameToLayer("MovingOneWayPlatforms");
        MidHeightOneWayPlatformMask = 1<<LayerMask.NameToLayer("MidHeightOneWayPlatforms");
    }
    /// <summary>
    ...
    protected virtual void FrameInitialization()
    {
        ...
        NormalState.WasGroundedLastFrame = NormalState.IsCollidingBelow;
        StandingOnLastFrame = StandingOn;
        NormalState.WasTouchingTheCeilingLastFrame = NormalState.IsCollidingAbove;
        CurrentWallCollider = null;
        NormalState.Reset();
    }
    ...
    protected virtual void HandleMovingPlatforms()
    {
        if (_movingPlatform != null)
        {
            ...
            NormalState.OnAMovingPlatform = true;
    ...
    public virtual void DetachFromMovingPlatform()
    {
        ...
        GravityActive(true);
        NormalState.OnAMovingPlatform = false;
        ...
        switch (direction)
        {
            case RaycastDirections.left:
                ...
                for (int i = 0; i < NumberOfHorizontalRays; i++)
                {
                    ...
                    storageArray[i] = PhysicsDebugger.RayCast(rayOriginPoint, -transform.right, rayLength, PlatformMask & ~OneWayPlatformMask & ~MovingOneWayPlatformMask, color, Parameters.DrawRaycastsGizmos);
                    if (storageArray[i])
                    {
                        returnValue = true;
                    }
            ...
            case RaycastDirections.down:
                ...
                for (int i = 0; i < NumberOfVerticalRays; i++)
                {
                    ...
                    if ((_newPosition.y < 0) && (!NormalState.WasGroundedLastFrame))
                    {
                        storageArray[i] = PhysicsDebugger.RayCast(rayOriginPoint, -transform.up, rayLength, PlatformMask & ~OneWayPlatformMask & ~MovingOneWayPlatformMask, color, Parameters.DrawRaycastsGizmos);
                        if (storageArray[i])
                        {
                            returnValue = true;
                        }
    ...
    protected virtual void CastRaysToTheSides(float raysDirection)
    {
        ...
        for (int i = 0; i < NumberOfHorizontalRays; i++)
        {
            ...
            if (NormalState.WasGroundedLastFrame && i == 0)
            {
                _sideHitsStorage[i] = PhysicsDebugger.RayCast(rayOriginPoint, raysDirection * (transform.right), horizontalRayLength, PlatformMask & ~OneWayPlatformMask & ~MovingOneWayPlatformMask, Color.red, Parameters.DrawRaycastsGizmos);

                //_sideHitsStorage[i] = PhysicsDebugger.RayCast(rayOriginPoint, raysDirection * (transform.right), horizontalRayLength, PlatformMask,Color.blue, Parameters.DrawRaycastsGizmos);
            }
            else
            {
                _sideHitsStorage[i] = PhysicsDebugger.RayCast(rayOriginPoint, raysDirection * (transform.right), horizontalRayLength, PlatformMask & ~OneWayPlatformMask & ~MovingOneWayPlatformMask, Color.red, Parameters.DrawRaycastsGizmos);
            ...
            if (_sideHitsStorage[i].distance > 0)
            {
                ...
                {
                    NormalState.LateralSlopeAngle = hitAngle;
                }
                ...
                if (hitAngle > Parameters.MaximumSlopeAngle)
                {
                    if (raysDirection < 0)
                    {
                        NormalState.IsCollidingLeft = true;
                        NormalState.DistanceToLeftCollider = _sideHitsStorage[i].distance;
                    }
                    else
                    {
                        NormalState.IsCollidingRight = true;
                        NormalState.DistanceToRightCollider = _sideHitsStorage[i].distance;
                    }

                    if (_movementDirection == raysDirection)
                    {
                        CurrentWallCollider = _sideHitsStorage[i].collider.gameObject;
                        NormalState.SlopeAngleOK = false;

                        float distance = MathUtils.DistanceBetweenPointAndLine(_sideHitsStorage[i].point, _horizontalRayCastFromBottom, _horizontalRayCastToTop);
                        if (raysDirection <= 0)
                        {
                        ...
                        if (!NormalState.IsGrounded && (Speed.y != 0))
                        {
                            _newPosition.x = 0;
                        }
    ...
    protected virtual void CastRaysBelow()
    {
        ...
        {
            NormalState.IsFalling = true;
        }
        else
        {
            NormalState.IsFalling = false;
        }

        if ((Parameters.Gravity > 0) && (!NormalState.IsFalling))
        {
            NormalState.IsCollidingBelow = false;
            return;
        }

        //float rayLength = _boundsHeight / 2 + RayOffset;
        float rayLength = 0.05f + 0.05f;

        if (NormalState.OnAMovingPlatform)
        {
            rayLength *= 2;
        }
        ...
        _verticalRayCastFromLeft = (_boundsBottomLeftCorner + _boundsTopLeftCorner) / 2 + _boundsHeight / 2 * Vector2.down;
        _verticalRayCastToRight = (_boundsBottomRightCorner + _boundsTopRightCorner) / 2 + _boundsHeight / 2 * Vector2.down;

        //_verticalRayCastFromLeft = (_boundsBottomLeftCorner + _boundsTopLeftCorner) / 2;
        //_verticalRayCastToRight = (_boundsBottomRightCorner + _boundsTopRightCorner) / 2;
        _verticalRayCastFromLeft += (Vector2)transform.up * 0.05f;
        _verticalRayCastToRight += (Vector2)transform.up * 0.05f;
        _verticalRayCastFromLeft += (Vector2)transform.right * _newPosition.x;
        _verticalRayCastToRight += (Vector2)transform.right * _newPosition.x;
        ...
        //    _belowHitsStorage = new RaycastHit2D[NumberOfVerticalRays];
        //}
        if (_belowHitsAllStorage.Length != NumberOfVerticalRays)
        {
            _belowHitsAllStorage = new RaycastHit2D[NumberOfVerticalRays][];
        }
        ...
        if (NormalState.WasGroundedLastFrame)
        {
            if (StandingOnLastFrame != null)
            {
                if (!MidHeightOneWayPlatformMask.Contains(StandingOnLastFrame.layer))
        ...
        float smallestCross = 0f;
        int smallestDistanceIndex = 0;
        int smallestFirstValidIdx = 0;
        ...
        for (int i = 0; i < NumberOfVerticalRays; i++)
        {
            ...
            if ((_newPosition.y > 0) && (!NormalState.WasGroundedLastFrame))
            {
                //_belowHitsStorage[i] = PhysicsDebugger.RayCast(rayOriginPoint, -transform.up, rayLength, _raysBelowLayerMaskPlatformsWithoutOneWay, Color.blue, Parameters.DrawRaycastsGizmos);
                _belowHitsAllStorage[i] = PhysicsDebugger.RayCastAll(rayOriginPoint, -transform.up, rayLength, _raysBelowLayerMaskPlatformsWithoutOneWay, Color.blue, Parameters.DrawRaycastsGizmos);
            }
            else
            {
                //_belowHitsStorage[i] = PhysicsDebugger.RayCast(rayOriginPoint, -transform.up, rayLength, _raysBelowLayerMaskPlatforms, Color.blue, Parameters.DrawRaycastsGizmos);
                _belowHitsAllStorage[i] = PhysicsDebugger.RayCastAll(rayOriginPoint, -transform.up, rayLength, _raysBelowLayerMaskPlatforms, Color.blue, Parameters.DrawRaycastsGizmos);
            }

            if (!NormalState.WasGroundedLastFrame)
            {
                Array.Sort(_belowHitsAllStorage[i], (x, y) =>
            ...
            //if (_belowHitsAllStorage[smallestDistanceIndex].Length > 0)
            //{
            //    distance = MathUtils.DistanceBetweenPointAndLine(_belowHitsAllStorage[smallestDistanceIndex][0].point, _verticalRayCastFromLeft, _verticalRayCastToRight);
            //}
            ...
            //}
            int validIdx = 0;
            while(validIdx< _belowHitsAllStorage[i].Length && _belowHitsAllStorage[i][validIdx].collider == _ignoredCollider)
            {
                validIdx++;
            }
            if(validIdx >= _belowHitsAllStorage[i].Length)
            {
                continue;
            }
            float distance = MathUtils.DistanceBetweenPointAndLine(_belowHitsAllStorage[i][validIdx].point, _verticalRayCastFromLeft, _verticalRayCastToRight);
            ...
            NormalState.BelowSlopeAngle = Vector2.Angle(_belowHitsAllStorage[i][validIdx].normal, transform.up);
            _crossBelowSlopeAngle = Vector3.Cross(transform.up, _belowHitsAllStorage[i][validIdx].normal);

            //NormalState.BelowSlopeAngle = Vector2.Angle(_belowHitsStorage[i].normal, transform.up);
            //_crossBelowSlopeAngle = Vector3.Cross(transform.up, _belowHitsStorage[i].normal);
            if (_crossBelowSlopeAngle.z < 0)
            {
                NormalState.BelowSlopeAngle = -NormalState.BelowSlopeAngle;
            }
            if (_belowHitsAllStorage[i][validIdx].distance < smallestDistance)
            {
                smallestDistanceIndex = i;
                smallestFirstValidIdx = validIdx;
                smallestDistance = _belowHitsAllStorage[i][validIdx].distance;
                smallestCross = _crossBelowSlopeAngle.z;
            }
        ...
        if (hitConnected)
        {
            StandingOn = _belowHitsAllStorage[smallestDistanceIndex][smallestFirstValidIdx].collider.gameObject;
            StandingOnCollider = _belowHitsAllStorage[smallestDistanceIndex][smallestFirstValidIdx].collider;
            ...
            if (
                !NormalState.WasGroundedLastFrame
                //&& (smallestDistance < _boundsHeight / 2)
                && (smallestDistance < 0.01f)
                && (
            ...
            NormalState.IsFalling = false;
            NormalState.IsCollidingBelow = true;
            ...
            {
                float distance = _belowHitsAllStorage[smallestDistanceIndex][smallestFirstValidIdx].distance;

                //Debug.Log(StandingOn);
                //if (_belowHitsStorage[smallestDistanceIndex].collider.GetComponent<MapFootHold>().isSlope()){
                if (_newPosition.x == 0 || _newPosition.x * smallestCross < 0)
                    {
                        if(-distance + 0.05f > 0)
                        {
                            _newPosition.y = 0;
                ...
                //if (StandingOnLastFrame == null || _belowHitsAllStorage[smallestDistanceIndex][0].collider.gameObject == StandingOnLastFrame)
                //{
                //    //新落地 或 在同一平台上移动 则一定调整
                //    //MathUtils.DistanceBetweenPointAndLine(_belowHitsStorage[smallestDistanceIndex].point, _verticalRayCastFromLeft , _verticalRayCastToRight);
                //    _newPosition.y = -distance
                //        //+ _boundsHeight / 2
                //        //+ RayOffset;
                //        + 0.05f;
                ...
                //   //落差太大 不调整
                //}
                //else if(!_belowHitsAllStorage[smallestDistanceIndex][0].collider.GetComponent<MapFootHold>().isSlope())
                //{
                //    //非斜坡 调整
                //    _newPosition.y = -distance
                //        //+ _boundsHeight / 2
                ...
                //        //+ RayOffset;
                //        + 0.05f;
                //    //bool changePlat = true;
                //    //for(int i=0; i < _belowHitsAllStorage[smallestDistanceIndex].Length; i++)
                //    //{
                //    //    if(_belowHitsAllStorage[smallestDistanceIndex][i].collider.gameObject == StandingOn)
                //    //    {
                //    //        changePlat = false;
                //    //        Debug.Log("diff platform ");
            ...
            if (!NormalState.WasGroundedLastFrame && _speed.y > 0)
            {
                _newPosition.y += _speed.y * Time.deltaTime;
            }
            ...
            _frictionTest = _belowHitsAllStorage[smallestDistanceIndex][smallestFirstValidIdx].collider.gameObject.GetComponent<SurfaceModifier>();
            if (_frictionTest != null)
            {
                _friction = _belowHitsAllStorage[smallestDistanceIndex][smallestFirstValidIdx].collider.GetComponent<SurfaceModifier>().Friction;
            }

            // we check if the character is standing on a moving platform
            _movingPlatformTest = _belowHitsAllStorage[smallestDistanceIndex][smallestFirstValidIdx].collider.gameObject.GetComponent<MovingPlatform>();
            if (_movingPlatformTest != null && NormalState.IsGrounded)
            {
                _movingPlatform = _movingPlatformTest.GetComponent<MovingPlatform>();
            }
            else
            {
            ...
            NormalState.GroundThisFrame = _belowHitsAllStorage[smallestDistanceIndex][smallestFirstValidIdx].collider.gameObject;
        }
        else
        {
            NormalState.IsCollidingBelow = false;
            ...
            NormalState.GroundThisFrame = null;
        }
    ...
    protected virtual void CastRaysAbove()
    {
        ...
        float rayLength = NormalState.IsGrounded ? RayOffset : _newPosition.y;
        ...
        for (int i = 0; i < NumberOfVerticalRays; i++)
        {
            ...
            _aboveHitsStorage[i] = PhysicsDebugger.RayCast(rayOriginPoint, (transform.up), rayLength, PlatformMask & ~OneWayPlatformMask & ~MovingOneWayPlatformMask, Color.cyan, Parameters.DrawRaycastsGizmos);
        ...
        if (hitConnected)
        {
            ...
            if ((NormalState.IsGrounded) && (_newPosition.y < 0))
            {
                _newPosition.y = 0;
            }

            NormalState.IsCollidingAbove = true;

            if (!NormalState.WasTouchingTheCeilingLastFrame)
            {
    ...
    protected virtual void StickToSlope()
    {
        ...
        _stickRaycast = PhysicsDebugger.RayCast(_stickRayCastOrigin, -transform.up, rayLength, PlatformMask, Color.blue, Parameters.DrawRaycastsGizmos);
    ...
    protected virtual void ComputeNewSpeed()
    {
        ...
        if (NormalState.IsGrounded)
        {
            _speed.x *= Parameters.SlopeAngleSpeedFactor.Evaluate(Mathf.Abs(NormalState.BelowSlopeAngle) * Mathf.Sign(_speed.y));
        }
    ...
    protected virtual void SetStates()
    {
        ...
        if (!NormalState.WasGroundedLastFrame && NormalState.IsCollidingBelow)
        {
            NormalState.JustGotGrounded = true;
        }

        if (NormalState.IsCollidingLeft || NormalState.IsCollidingRight || NormalState.IsCollidingBelow || NormalState.IsCollidingAbove)
        {
            OnCorgiColliderHit();
        }
    }
    ...
    public virtual void TryDisableCollisionsWithOneWayPlatforms(float time)
    {
        StartCoroutine(DisableCollisionsWithOneWayPlatforms(time));
    }
    ...
    public virtual void TryDisableCollisionsWithMovingPlatforms(float time)
    {
        StartCoroutine(DisableCollisionsWithMovingPlatforms(time));
    }
    /// <summary>
    ...
    public virtual void ResizeCollider(Vector2 newSize)
    {
        ...
        SetRaysParameters();
        NormalState.ColliderResized = true;
    ...
    public virtual void ResetColliderSize()
    {
        ...
        SetRaysParameters();
        NormalState.ColliderResized = false;
    }
    ...
    public virtual bool CanGoBackToOriginalSize()
    {
        ...
        bool headCheckLeft = PhysicsDebugger.RayCast(_originalSizeRaycastOrigin, transform.up, headCheckDistance, PlatformMask - OneWayPlatformMask, Color.gray, true);

        _originalSizeRaycastOrigin = _boundsTopRightCorner + (Vector2)transform.up * _smallValue;
        bool headCheckRight = PhysicsDebugger.RayCast(_originalSizeRaycastOrigin, transform.up, headCheckDistance, PlatformMask - OneWayPlatformMask, Color.gray, true);
